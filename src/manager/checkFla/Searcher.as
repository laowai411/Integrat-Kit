package manager.checkFla
{
	...
	public class Searcher extends EventDispatcher
	{
		...
		{
			linkList = cusList;
			codeSrcURL = new File(CheckFlaConfig.codeSrcURL);
			
			GameDispatcher.getInstance().dispatchEvent(new ParamEvent(EventName.PRINT, "开始载入代码..."));
			getCodeURL(codeSrcURL);
			codeFileCount = codeURLList.length;
			
			GameDispatcher.getInstance().dispatchEvent(new ParamEvent(EventName.PRINT, "代码载入完成，开始搜索..."));
			
			_useLinkMap = new Dictionary();
			
			timer = new Timer(CheckFlaConfig.searchDelay);
		...
		private function onFrameSearchHandler(e:TimerEvent):void
		{
			...
			while(true)
			{
				...
				if(getTimer()-startTime > CheckFlaConfig.searchMaxTime)
				{
					break;
				}
				var file:File = codeURLList.pop();
				searchAS(file);
				GameDispatcher.getInstance().dispatchEvent(new ParamEvent(EventName.CHANGE_STATE, {typeName:"代码文件", total:codeFileCount, odd:codeURLList.length}));
			}
		}
		...
		private function searchAS(file:File):void
		{
			...
			GameDispatcher.getInstance().dispatchEvent(new ParamEvent(EventName.PRINT, file.nativePath+" 搜索完成!"));
		}
